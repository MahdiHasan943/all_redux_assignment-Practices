import React, { useEffect, useState } from 'react'
import { useDispatch, useSelector } from 'react-redux';
import { getProducts, removePRoduct, toggleDeleteSuccess } from '../../features/products/ProductSlice';
import Update from '../../components/Update';
import toast, { Toaster } from 'react-hot-toast';


const AllPList = () => {
  
  const { products ,isLoading,deleteSuccess} = useSelector((state) => state.product)
  
  const dispatch = useDispatch()
console.log(products);
  
  useEffect(() => {
    dispatch(getProducts())
  }, [])


  useEffect(() => {
    if (!isLoading && deleteSuccess) {
      toast.success('delete successfull')
      dispatch(toggleDeleteSuccess())
    }

  }, [isLoading, deleteSuccess])
  if (isLoading) {
     <p>Loading....</p>;
  }

  const [activeProductId, setActiveProductId] = useState(null);

  const handleEditClick = (productId) => {
    setActiveProductId(productId);
  };


  return (
    <div className='relative py-4 px-4 sm:px-16  lg:w-[65%]'>

      <table className='w-full'>
        <thead className=''>
          <tr className='py-4 bg-red-50 px-4 rounded-md w-full grid grid-cols-5 gap-8'><td>Name</td>
          <td>image</td>
      <td>Price</td>
        <td>Delete</td>
          <td>Edit</td></tr>

        </thead>
        <tbody>
          {
            products.map(({  image, price, model, _id }) => (
              <tr className='py-4 bg-red-50 px-4 rounded-md w-full grid grid-cols-5 gap-8'><td>{model}</td> <td><img className='w-[80px] h-[80px]' src={image} alt="" /></td>
                <td>{price}</td> <td>
                  <button onClick={() => dispatch(removePRoduct(_id))}>
                  <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 30 30">
    <path d="M 14.984375 2.4863281 A 1.0001 1.0001 0 0 0 14 3.5 L 14 4 L 8.5 4 A 1.0001 1.0001 0 0 0 7.4863281 5 L 6 5 A 1.0001 1.0001 0 1 0 6 7 L 24 7 A 1.0001 1.0001 0 1 0 24 5 L 22.513672 5 A 1.0001 1.0001 0 0 0 21.5 4 L 16 4 L 16 3.5 A 1.0001 1.0001 0 0 0 14.984375 2.4863281 z M 6 9 L 7.7929688 24.234375 C 7.9109687 25.241375 8.7633438 26 9.7773438 26 L 20.222656 26 C 21.236656 26 22.088031 25.241375 22.207031 24.234375 L 24 9 L 6 9 z"></path>
                </svg>
                  </button>
                  </td>
                <td> <button
                  onClick={() => handleEditClick(_id)}
                >Edit</button>
                {activeProductId === _id && (
                  <div className='absolute flex justify-center items-center h-[100vh] w-full shadow-2xl bg-slate-50 top-0 left-0'>
                    <p
                      onClick={() => setActiveProductId(null)}
                      className='absolute top-4 right-4 text-right text-4xl font-bold text-indigo-600'
                    >
                      x
                    </p>
                    <Update  _id={_id}  />
                  </div>
                )}
                </td>
              </tr>
            ))
          }
  
        </tbody>
      </table>
      <Toaster/>
    </div>
  )
}

export default AllPList